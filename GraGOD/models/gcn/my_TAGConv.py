...
from models.gcn.my_gcn_conv import gcn_norm
...
class TAGConv(MessagePassing):
    ...
    def __init__(self, in_channels: int, out_channels: int, K: int = 3,
                 bias: bool = True, normalize: bool = True, direct: bool = True, **kwargs):
        ...
        if self.normalize:
            if isinstance(edge_index, Tensor):
                edge_index, edge_weight = gcn_norm(  # yapf: disable
                    ...
                    dtype=x.dtype, direct=direct)

            elif isinstance(edge_index, SparseTensor):
                edge_index = gcn_norm(  # yapf: disable
                    edge_index, edge_weight, x.size(self.node_dim),
                    add_self_loops=False, flow=self.flow, dtype=x.dtype, direct=direct)
